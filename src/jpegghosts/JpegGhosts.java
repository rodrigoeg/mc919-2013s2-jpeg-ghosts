...
public class JpegGhosts {
	public void simplerVersion(final File fileEntry, String folderDestination) {
    	// Read an image
		Mat image = Highgui.imread(fileEntry.getPath());

		ExecutorService executor = Executors.newFixedThreadPool(4);
		for (int quality = 20; quality <= 100; quality = quality + 1) {
    ...
    public void fullVersion(final File fileEntry, int blockSize, String folderDestination) {
		// Read an image
		Mat image = Highgui.imread(fileEntry.getPath());
		
		ExecutorService executor = Executors.newFixedThreadPool(4);
		for (int quality = 20; quality <= 100; quality = quality + 5) {
